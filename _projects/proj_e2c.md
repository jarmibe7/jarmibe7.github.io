...
My implementation allows predictions to be based on an observation history rather than just a single image, which enabled crisper and more accurate predictions.

## Results
The first video on this page is a visualization of the performance. The example uses the Reacher environment, which is arguably a more complicated system than the 2D examples in the original E2C paper. This may be a result of including observation history instead of a single image, but further study would be required for substantive analysis. 
...
In the figure above, the 2D joint spaces between each latent variable are visualized in a grid. It is unclear whether there is a spatial correlation between the latent space and the original Reacher dynamics, but there do seem to be some radial symmetry common to both the physical and latent representations.

## Future Work
For my Master's thesis, I am examining the potential of intelligent closed-loop active learning for learning dynamics models like E2C. I aim to show that active learning can decrease the amount of data needed to learn a dynamics model.
